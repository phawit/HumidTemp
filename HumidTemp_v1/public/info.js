...
curr_ref.once('value',function (snapshot) {
  ...
  document.getElementById("unitname").innerHTML = 'หน่วย: ' + data['UnitName'];
});
...
var x_dropdown = "1 ชม.ที่ผ่านมา,3 ชม.ที่ผ่านมา,6 ชม.ที่ผ่านมา,12 ชม.ที่ผ่านมา,24 ชม.ที่ผ่านมา,สัปดาห์ที่ผ่านมา,เดือนที่ผ่านมา";
var chart_last_title = ["1 ชม.ที่ผ่านมา","3 ชม.ที่ผ่านมา","6 ชม.ที่ผ่านมา","12 ชม.ที่ผ่านมา","24 ชม.ที่ผ่านมา","สัปดาห์ที่ผ่านมา","เดือนที่ผ่านมา"];
var options = x_dropdown.split(",");
var select = document.getElementById('myoptions');
// select.options[0] = new Option('Choose View', '');
select.options[0] = new Option('เลือกช่วงเวลา', '');
...
function CreateChart(data_plot, chart, plot_last) {
  ...
  td.appendChild(document.createTextNode('เวลา'));
  tr.appendChild(td);
  ...
  td.appendChild(document.createTextNode('ดัชนีความร้อน (°C)'));
  tr.appendChild(td);
  ...
  chart = new CanvasJS.Chart("chartContainer", {
    ...
    title:{
      ...
      text: "อุณหภูมิ และความชื้นสัมพัทธ์ (" + chart_last_title[title_idx] + ')',
      fontSize: 20
    },	
    axisX:{
