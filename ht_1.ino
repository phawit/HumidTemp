/*
 *DHT22<-->D6  
 *SWITCH<-->D7 //GPIO 13 **increse
 *SWITCH<-->D5 //GPIO 14 **setting
 *SWITCH<-->D8   //GPIO 15 **decrese, hospot
 *SCL<-->D1
 *SDA<-->D2
 *LED_online<-->D3<-->3V  //yellow
 *
*/
...
const int interruptPinSetting = 14;    //setting
const int interruptPinIncrese = 13;   //increse
const int interruptPin = 15;    //decrese, hospot

const int LED_online=D3;    //16;   //D0 led out
...
void setup() {
  ...
  pinMode(interruptPinSetting, INPUT); 
  pinMode(interruptPinIncrese, INPUT); 
  
  attachInterrupt(digitalPinToInterrupt(interruptPin), handleInterrupt, RISING); //RISING
  attachInterrupt(digitalPinToInterrupt(interruptPinSetting), handleInterruptSetting, RISING);
  attachInterrupt(digitalPinToInterrupt(interruptPinIncrese), handleInterruptIncrese, CHANGE); 
...
void loop() {
  ...
  while(sta2==1){
    ...
    while(staSet==0){
      ...
      while(digitalRead(interruptPinIncrese)==1){
      ...
      Serial.println(digitalRead(interruptPinIncrese));
...
void handleInterruptSetting() { 
    ...
    time_start = millis();
      while(digitalRead(interruptPinSetting)==1){
        if(millis()-time_start > 3000){
...
void handleInterruptIncrese() { 
    ...
    if(sta2==1){
      ...
      if(staSet==1){
        staHumid = staHumid+0.01;
        Serial.print("\tHumid: "); Serial.print(humid+staHumid);
        lcd.setCursor(8, 2); lcd.print(humid+staHumid); lcd.setCursor(14, 2); lcd.print("h"); lcd.print(staHumid);
      }
